...
const FeaturesSection: FC = () => {
  ...
  {features.map((feature, index) => (
    <div
      key={feature.title}
      className={`bg-yellow-400 text-left rounded-lg p-6 shadow-md transform hover:scale-105 transition-transform duration-300 ${
        index === 0 ? '-rotate-6' : '' // Add anti-clockwise rotation to the first card
      }`}
    >
